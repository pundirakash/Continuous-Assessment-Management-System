...
const deleteSet = async () => {
    ...
    try {
        // 1. Find Faculty
        const facultyName = "Akash pundir"; // Case sensitive search usually, but let's try regex
        ...
        if (!assessment) {
            ...
            console.log("Available Assessments:", allAssessments.map(a => a.name));
            process.exit(1);
        }
        console.log(`Found Assessment: ${assessment.name} (${assessment._id})`);

        // 4. Find Hierarchy
        const facultyIndex = assessment.facultyQuestions.findIndex(fq => fq.faculty.toString() === faculty._id.toString());
        if (facultyIndex === -1) {
            console.log(`Faculty ${faculty.name} has no submissions in this assessment.`);
            process.exit(1);
        }
        ...
        const questionIds = payment = setToDelete.questions;
